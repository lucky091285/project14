...
module.exports.createCard = (req, res) => {
    ...
    Card.create({ name, link, owner })
        ...
        .catch(err => res.status(500).send({ message: `Произошла ошибка добавлении карточки` }));
};
...
module.exports.getAllCards = (req, res) => {
  Card.find({})
        .populate('owner')
        .then(card => res.send({ data: card }))
        .catch(err => res.status(500).send({ message: `Произошла ошибка при загрузке карточек` }));
};
